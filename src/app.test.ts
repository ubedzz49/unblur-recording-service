...
function setup(bookingStatus = "completed") {
  ...
  bookingClient.seed({ id: BOOKING_ID, posterUserId: POSTER, resolverUserId: RESOLVER, status: bookingStatus });
  const app = buildApp(repo, bookingClient, INTERNAL_TOKEN);
  return { app, repo, bookingClient };
}
